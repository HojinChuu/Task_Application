...
export default {
	...
	methods: {
		...
		async getCompleted() {
			const headers = {'Authorization': this.access_token};
			const response = await axios.get("http://localhost/restapi/tasks/complete", { headers });
			this.tasks = await response.data.data.tasks;
			this.tasks_count = await response.data.data.rows_returned;
		},
		async getIncompleted() {
			const headers = {'Authorization': this.access_token};
			const response = await axios.get("http://localhost/restapi/tasks/incomplete", { headers });
			this.tasks = await response.data.data.tasks;
			this.tasks_count = await response.data.data.rows_returned;
		}
	}
