			...
			<form @submit="onSubmit">
				<ion-list class="full">
					...
					<ion-item >
						...
						<ion-input v-else :value="title" @input="title = $event.target.value" required type="text" autofocus="true"></ion-input>
					</ion-item>
					<ion-item>
						<ion-label>Deadline : </ion-label>
						<ion-label v-if="!isEdit">{{ task_info.deadline }}</ion-label>
						<ion-datetime v-else display-format="YYYY-MM-DD HH:mm" :min="current_date" :max="max_date"></ion-datetime>
					</ion-item>
					<ion-item>
						<ion-label>Completed : </ion-label>
						<ion-label v-if="!isEdit">{{ task_info.completed }}</ion-label>
						<ion-input v-else :value="completed" @input="completed = $event.target.value" required type="text"></ion-input>
					</ion-item>
					...
					<ion-item>
						...
						<ion-textarea v-else :value="description" @input="description = $event.target.value" rows="6" cols="20" placeholder="Enter any notes here..."></ion-textarea>
					</ion-item>

				</ion-list>
                <ion-button v-show="isEdit" expand="block" color="secondary" type="submit">Update</ion-button>
			</form>
		</ion-content>
...
export default {
	...
	methods: {
		...
		onSubmit(e) {
			...
			axios.patch(`http://localhost/restapi/tasks/${this.task_info.id}`, data, { headers })
            .then(response => {
